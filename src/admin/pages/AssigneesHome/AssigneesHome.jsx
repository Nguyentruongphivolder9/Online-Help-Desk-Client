import React, { useState } from 'react'
import HeaderAdmin from '@/admin/components/HeaderAdmin';
import useAuthRedirect from '@/hooks/useAuthRedirect';
import LoadingOverlay from '@/common/components/LoadingOverlay';
import LobbyChat from '@/common/components/LobbyChat/LobbyChat';
import SkeletonLoaderRequest from '@/common/components/SkeletonLoaderRequest';

export default function AssigneesHome() {
  const { accountId, isLoading } = useAuthRedirect("Assignees");
  const [showLobby, setShowLobby] = useState("Request");
  const [isShowResultSearch, setIsShowResultSearch] = useState(false);


  const handleSearchChange = (e) => {
    const value = e.target.value;
    if (value !== null && value !== '') {
      setIsShowResultSearch(true);
    } else {
      setIsShowResultSearch(false);
    }
  }

  return (
    <>
      {isLoading ? (
        <LoadingOverlay />
      ) : (
        <div>
          <HeaderAdmin urlLogo='/admin/assignees' container='container' accountId={accountId} />
          <div className="flex flex-row w-full h-full fixed justify-center overflow-hidden bg-white pt-[72px]">
            <div className='flex h-full flex-row w-full border border-x-gray-300'>
              <div className='h-full w-1/4 overflow-hidden'>
                <div className='w-full flex flex-row justify-between items-center p-[10px]'>
                  <div className="relative flex items-center w-11/12 h-[35px] rounded-lg border focus-within:border-gray-500 bg-slate-200 overflow-hidden">
                    <div className="grid place-items-center h-full w-12 text-gray-300">
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                      </svg>
                    </div>
                    <input
                      className="peer h-full w-full outline-none text-sm text-gray-700 pr-2 bg-slate-200"
                      type="text"
                      id="search"
                      placeholder="Search something.."
                      onChange={handleSearchChange}
                    />
                  </div>
                  <div className="grid place-items-center h-full w-12 text-gray-500">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M6 13.5V3.75m0 9.75a1.5 1.5 0 0 1 0 3m0-3a1.5 1.5 0 0 0 0 3m0 3.75V16.5m12-3V3.75m0 9.75a1.5 1.5 0 0 1 0 3m0-3a1.5 1.5 0 0 0 0 3m0 3.75V16.5m-6-9V3.75m0 3.75a1.5 1.5 0 0 1 0 3m0-3a1.5 1.5 0 0 0 0 3m0 9.75V10.5" />
                    </svg>
                  </div>
                </div>

                {isShowResultSearch ? (
                  <div className='relative w-full h-full text-sm'>
                    <div className='flex flex-row w-full p-[15px] items-center border-b gap-2 border-solid text-gray-700' >
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 0 0 2.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 0 0-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75 2.25 2.25 0 0 0-.1-.664m-5.8 0A2.251 2.251 0 0 1 13.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25ZM6.75 12h.008v.008H6.75V12Zm0 3h.008v.008H6.75V15Zm0 3h.008v.008H6.75V18Z" />
                      </svg>
                      <div className='p-[10px]text-base font-medium'>
                        Search Results
                      </div>
                    </div>

                    <div className='w-full h-full'>
                      <SkeletonLoaderRequest />
                      <SkeletonLoaderRequest />
                      <SkeletonLoaderRequest />
                      <SkeletonLoaderRequest />
                      <SkeletonLoaderRequest />
                      <SkeletonLoaderRequest />
                    </div>
                  </div>
                ) : (
                  <div className='relative w-full h-full text-sm'>
                    <div className='flex flex-row w-full' >
                      <button onClick={() => setShowLobby("Request")} className={`w-2/4  p-[10px] border-b-4 border-solid ${showLobby === "Request" ? "border-blue-500 text-blue-700" : "text-gray-700 border-gray-200"}`}>
                        Request
                      </button>
                      <button onClick={() => setShowLobby("Remark")} className={`w-2/4 p-[10px] text-sm border-b-4 border-solid ${showLobby === "Remark" ? "border-blue-500 text-blue-700" : "text-gray-700 border-gray-200"}`}>
                        Remark
                      </button>
                    </div>

                    {/* lobby */}
                    <LobbyChat itemName={showLobby} dataItem={""} />

                  </div>
                )}
              </div>
              <div id="main-content" className=" text-gray-700 h-full w-2/4 bg-red-50 relative overflow-y-scroll hide-scrollbar border-x border-x-gray-300">
                assignees.
              </div>
              <div className='relative text-gray-700 z-20 h-full top-0 flex lg:flex flex-shrink-0 flex-col w-1/4 transition-width duration-75 overflow-y-scroll hide-scrollbar'>
                <div className='w-full h-96 rounded-lg bg-red-800'>

                </div>
              </div>
            </div>
          </div>
        </div >
      )
      }
    </>
  )
}
